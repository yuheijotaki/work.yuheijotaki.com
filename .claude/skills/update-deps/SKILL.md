...
このプロジェクト（Next.js 16 App Router + microCMS + Vercel、npm 管理）の依存パッケージを安全に更新する。
...
- `.env` が無い: microCMS への接続が必要なため、欠けていれば「`MICROCMS_SERVICE_ID` / `MICROCMS_API_KEY` が設定されていません。build 時にも参照されます。設定してから再開しますか」と確認
...
### このプロジェクトで特に注意するパッケージ

major を伴うとき、breaking change が広範囲に波及しやすいので必ず CHANGELOG を読む:
...
- `markuplint` / `@markuplint/*` — `.markuplintrc` のルール名・スキーマ
...
| 指定形式                          | 例                            | 扱い                                                |
| --------------------------------- | ----------------------------- | --------------------------------------------------- |
| `^x.y.z`（caret 付き）            | `"minimatch": "^10.2.4"`      | **更新対象**。同 major の最新 patch+minor に bump   |
...
```bash
# 例: minimatch の ^10 系列の最新版
npm view 'minimatch@^10' version --json | jq -r 'if type=="array" then .[-1] else . end'
```

> **注意**: `--json` を付けないと、マッチが複数あるときに `minimatch@10.2.4 '10.2.4'` のような複数行の整形出力になり、単一バージョンとして扱えない。マッチが 1 件のときだけ単独文字列が返るので、`--json` + jq の上記イディオムで両ケースを統一して扱う。

- 書き換え例: `^10.2.4` → `^10.2.5`。**メジャーは触らない**
...
- 特に `react` / `react-dom` / `next` は互いに peer 制約が強い。3 つは原則同時に上げる
...
```bash
until grep -q -E "Local|ready|error|started server on" <output-file>; do sleep 1; done
```
...
3. **dev server のバックグラウンドプロセスを必ず停止**する（孤立プロセス防止）
   ...
   停止後に `pgrep -f "next dev"` などで残存していないか確認する。
...
| dev server が残った                    | `pkill -f "next dev"` または KillShell でシェルID指定で停止 |
| `.next/` キャッシュが疑わしい          | `rm -rf .next` を提案（lockfile/node_modules には触らない） |
...
- `react` / `react-dom` / `next` の major を片方だけ上げる（peer 制約で破綻する）
- `.node-version` をこのスキルで書き換える（依存更新の範疇外）
